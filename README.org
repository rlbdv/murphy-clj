...
pending at the end of the cleanup will be thrown. It also allows
destructuring.
...
(with-final [foo (.acquire lock) :always .release
             {:keys [id kind] :as bar} (open something) :always .close
             baz {:foo foo :bar bar}]
  ...)
#+END_SRC clojure
...
This project is free software; unless otherwise specified below, you
...
Copyright © 2017-2018, 2020-2021 2025 Rob Browning <rlb@defaultvalue.org>

The with-final function in .clj-kondo/hooks.clj, which is only needed
by kondo for linting, is derived from the PuppetDB version as of

  commit 0c03037f659edf13307bc190bdc44f014aeb4158
  Author: Rob Browning <rlb@puppet.com>
  Date:   Mon Apr 18 11:28:35 2022 -0500

    (PDB-5484) Add clj-kondo dependency, cmd, and config

which was covered under the terms of the Apache License, Version 2.0,
